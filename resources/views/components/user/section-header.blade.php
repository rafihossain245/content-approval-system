    ...
    @else
        <div class="btn-toolbar d-block text-end" role="toolbar" aria-label="Toolbar with buttons">
            @if (Str::endsWith(Route::currentRouteName(), "index"))
                <x-user.buttons.return-back />

                @if (auth()->user()->can("add_" . $module_name) && Route::has("user." . $module_name . ".create"))
                    <x-user.buttons.create
                        title="{{ __('Create') }} {{ ucwords(Str::singular($module_name)) }}"
                        small="true"
                        route='{{ route("user.$module_name.create") }}'
                    />
                @endif

                @if (auth()->user()->can("restore_" . $module_name) && Route::has("user." . $module_name . ".create"))
                    <div class="btn-group">
                        <button
                        ...
                        <ul class="dropdown-menu">
                            <li>
                                <a class="dropdown-item" href="{{ route("user.$module_name.trashed") }}">
            ...
            @elseif (Str::endsWith(Route::currentRouteName(), "create"))
                <a
                    ...
                    data-toggle="tooltip"
                    href="{{ route("user.$module_name.index") }}"
            ...
            @elseif (Str::endsWith(Route::currentRouteName(), "show"))
                ...
                @if (auth()->user()->can("edit_" . $module_name) && Route::has("user." . $module_name . ".edit"))
                    <x-buttons.edit
                        class="m-1"
