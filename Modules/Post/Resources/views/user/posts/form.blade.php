...
<div class="row">
    <div class="col-12 col-sm-4 mb-3">
        <div class="form-group">
            ...
            {{ html()->label($field_lable, $field_name)->class("form-label")->for($field_name) }}
            {!! field_required($required) !!}
            {{ html()->select($field_name, $field_options, $selected)->placeholder($field_placeholder)->class("form-select select2-category-user")->attributes(["$required"]) }}
        </div>
    </div>
    <div class="col-12 col-sm-4 mb-3">
        <div class="form-group">
...
@push("after-scripts")
    ...
    <script type="module">
        $(document).ready(function () {
            ...
            $('.select2-category-user').select2({
                theme: 'bootstrap-5',
                placeholder: '@lang("Select an option")',
                minimumInputLength: 2,
                allowClear: true,
                ajax: {
                    url: '{{ route("user.categories.index_list") }}',
                    dataType: 'json',
            ...
            $('.select2-tags').select2({
                ...
                allowClear: true,
                ajax: {
                    url: '{{ route("user.tags.index_list") }}',
                    dataType: 'json',
                    data: function (params) {
                        return {
                            q: $.trim(params.term),
