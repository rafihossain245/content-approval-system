...
namespace Modules\Post\Http\Controllers\User;
...
class PostsController extends UserBaseController
{
    ...
    public function store(Request $request)
    {
        ...
        $data['status'] = PostStatus::Pending->value; // Set status to pending for approval

        $$module_name_singular = $module_model::create($data);
        $$module_name_singular->tags()->attach($request->input('tags_list'));

        flash("New '".Str::singular($module_title)."' Added and waiting for approval")->success()->important();
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($$module_name_singular->created_by !== auth()->id()) {
            flash('You are not authorized to update this post')->error()->important();
            return redirect("user/{$module_name}");
        }
        ...
        flash(Str::singular($module_title)."' Updated Successfully and waiting for approval")->success()->important();
